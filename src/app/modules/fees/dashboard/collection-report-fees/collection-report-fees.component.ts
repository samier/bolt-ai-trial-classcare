...
@Component({
  selector: 'app-collection-report-fees',
  templateUrl: './collection-report-fees.component.html',
  styleUrls: ['./collection-report-fees.component.scss']
})
export class CollectionReportFeesComponent implements OnInit, OnChanges {
  ...
  branchWiseChart(item) {
    ...
    if (item.name == 'All Branch') {
      chartData = this.collectionData.mode_wise_collection.map((ele) => {
        return { name: ele.payment_mode, y: ele.paid_amount }
      })
    } else {
      const branchData = this.collectionData.branch_wise_collection.find(ele => ele.branch_name == item.name);
      if (branchData) {
        chartData = Object.keys(branchData.modes).map(key => ({
          name: key,
          y: branchData.modes[key]
        }));
      } else {
  ...
  getFeesCollectionData() {
    ...
    this.isLoading = true
    this._feesService.getFeesCollection(payload).pipe(takeUntil(this.$destroy)).subscribe((res: any) => {
      if (res.status) {
        this.collectionData = res.data
        const chartData = res.data.mode_wise_collection.map((ele)=> {
          return { name: ele.payment_mode, y: ele.paid_amount }
        })
        this.totalCollection = chartData.reduce((total, item) => total + item.y,0 )
        this.updateChartData(chartData);
        const branch = this.collectionData.branch_wise_collection.map((ele, index) => {
