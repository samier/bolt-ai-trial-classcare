...
export class NotificationSettingComponent implements OnInit {
  ...
  selectedMenuName: string = 'General Notification'
  fieldData: any
  ...
  constructor(public CommonService: CommonService,
    ...
    this.notificationId = this._activatedRoute.snapshot.paramMap.get('id') || null
  }
  ...
  ngOnInit(): void {
    ...
    //   this.selectedMenu = Number(this.notificationId)
    // }
  ...
  saveData() {
    ...
    this._systemSettingService.saveNotificationSetting(this.payload).pipe(takeUntil(this.$destroy)).subscribe((res:any)=>{
  ...
  getMainMenuList() {
    ...
    this._systemSettingService.getNotificationMainMenuData().pipe(takeUntil(this.$destroy)).subscribe((res: any) => {
      if (res.status) {
  ...
  typeOnGetData() {
    ...
    if (this.notificationId) {
      this.selectedMenuName = this.systemMainMenuList.find((ele) => ele.type == this.notificationId)?.name ?? null
    }

    if (this.selectedMenuName) {
      this._systemSettingService.getNotificationMenuFieldData(payload).pipe(takeUntil(this.$destroy)).subscribe((res: any) => {
        if (res.status) {
