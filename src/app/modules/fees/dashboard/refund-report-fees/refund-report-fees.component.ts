...
@Component({
  selector: 'app-refund-report-fees',
  templateUrl: './refund-report-fees.component.html',
  styleUrls: ['./refund-report-fees.component.scss']
})
export class RefundReportFeesComponent implements OnInit, OnChanges {
  ...
  branchWiseChart(item) {
    ...
    if (item.name == 'All Branch') {
      chartData = this.refundData.mode_wise_collection.map((ele) => {
        return { name: ele.payment_mode, y: ele.total_amount }
      })
    } else {
      const branchData = this.refundData.branch_wise_collection.find(ele => ele.branch_name == item.name);
      if (branchData) {
  ...
  getFeesRefundData() {
    ...
    this.isLoading = true
    this._feesService.getFeesRefund(payload).pipe(takeUntil(this.$destroy)).subscribe((res: any) => {
      if (res.status) {
        ...
        const branch = this.refundData.branch_wise_collection.map((ele, index) => {
