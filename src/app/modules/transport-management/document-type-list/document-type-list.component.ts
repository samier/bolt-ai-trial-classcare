...
export class DocumentTypeListComponent {
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      stateSaveCallback: function(settings,data) {
        localStorage.setItem('DataTables_' + URLConstants.DOCUMENT_TYPE_LIST, JSON.stringify(data))
      },
      stateLoadCallback: function(settings) {
        const isModuleActive =  that.CommonService.isModuleActive(that.activatedRouteService.snapshot.data);
        if(isModuleActive) {
          let state:any = localStorage.getItem('DataTables_' +  URLConstants.DOCUMENT_TYPE_LIST)
  ...
  getlist(dataTablesParameters?: any, callback?:any ){
    this.transportService.getDocumentTypeList(dataTablesParameters).subscribe((resp:any) => {
  ...
  remove(id:any): void{
    if(confirm('are you sure you want to delete this document type ?')){
      this.transportService.deleteDocumentType(id).subscribe((res) => {  
        this.reloadData();
      }); 
    }
  }
