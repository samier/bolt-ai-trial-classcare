...
@Component({
  selector: 'app-faculty-requisition',
  templateUrl: './faculty-requisition.component.html',
  styleUrls: ['./faculty-requisition.component.scss']
})
export class FacultyRequisitionComponent implements OnInit {
  submitted:any=false;
  public valid = true;
  public roleList:any;
  itemTypeList:any;
      ...
      if(this.record_id != null && this.record_id !='' && this.record_id !=0){
        this.inventorySerivce.getRequisitionDetail(this.record_id).subscribe((res:any) => {
          if(res.status==false){
            ...
            // }      
            // //If its Sttudent
            // if(res.data.requisition_type==2){
...
getVendorTypeList(){  
  ...
  addRequisition(payload:any)
  {
    ...
    this.inventorySerivce.addRequisitionForFaculty(payload,record_id).subscribe((res:any) => {
      if(res.status==false){
        this.toastr.showError(res.message);
      }else{
        this.router.navigate([this.setUrl(URLConstants.FACULTY_REQUISITION_LIST)]);
      }    
    },(err:any)=>{
      this.toastr.showError(err.error.message);
    });    
